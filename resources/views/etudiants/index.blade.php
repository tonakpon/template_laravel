...
@section('content')
                ...
                @endif
                <h4>Affichage des etudiants enregistrés</h4>
                ...
                <table class="table table-striped">
                    ...
                    <tr>
                        ...
                        <td>{{ $item->tel }}</td>
                        <td>{{ $item->date }}</td>
                        <td>{{ $item->get_classe->libelle }}</td>
                        <td>
                            <form action="{{ route('etudiants.destroy',$item->id) }}" method="post">
                                <a class="btn btn-primary" href="{{ route('etudiants.edit',$item->id) }}"><i class="bi bi-pencil-square"></i></a>
                                @csrf
                                @method('DELETE')
...
<div class="modal fade" id="exampleModal_{{$item->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      Être vous sûrs de vouloir supprimer l'étudiant {{$item->name}} {{$item->fname}} ?
      </div>
